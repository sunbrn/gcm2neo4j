--------ITEM
\copy (SELECT 'i'||item_id as item_id,source_id,size,date,checksum,platform,pipeline,source_url,regexp_replace(local_url, E'\\n','') as local_url,'Item' as label FROM public.item) TO '../neo4j-community-3.4.1/import/gcm_entities/item.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (source_id,date,checksum,platform,pipeline,source_url,local_url));
--------ITEM2EXPERIMENT
...
--------DATASET
\copy (SELECT 'da'||dataset_id as dataset_id,regexp_replace(name, E'\\n','') as name,data_type,format,assembly,is_ann,annotation, 'Dataset' as label  FROM public.dataset) TO '../neo4j-community-3.4.1/import/gcm_entities/dataset.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (name, data_type,format,assembly,annotation));
--------CASE
\copy (SELECT 'c'||case_study_id as case_id,source_id,source_site,external_ref, 'Case' as label  FROM public.case_study) TO '../neo4j-community-3.4.1/import/gcm_entities/case.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (source_id,source_site,external_ref));
--------REPLICATE
\copy (SELECT 'r'||replicate_id as replicate_id,source_id,bio_replicate_num,tech_replicate_num, 'Replicate' as label  FROM public.replicate) TO '../neo4j-community-3.4.1/import/gcm_entities/replicate.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (source_id));
--------CASE2PROJECT
...
--------BIOSAMPLE
\copy (SELECT 'b'||biosample_id as biosample_id,source_id,type,tissue,cell_line,is_healthy,disease, 'Biosample' as label  FROM public.biosample)  TO '../neo4j-community-3.4.1/import/gcm_entities/biosample.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (source_id, type, tissue, cell_line,disease));
--------BIOSAMPLE2DONOR
...
--------DONOR
\copy (SELECT 'do'||donor_id as donor_id,source_id,species,age,gender,ethnicity, 'Donor' as label  FROM public.donor)  TO '../neo4j-community-3.4.1/import/gcm_entities/donor.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (source_id, species, ethnicity));
--------RELATION_HAS_TID
...
--------VOCABULARY
\copy (SELECT 'tid'||tid as tid,source,code,pref_label,regexp_replace(regexp_replace(regexp_replace(description, E'\\n\\n',' '), E'\\n',' '), E'\\s\\n',' ') as description,iri, 'Vocabulary' as label FROM public.vocabulary) TO '../neo4j-community-3.4.1/import/gcm_entities/vocabulary.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (source,code,pref_label,description,iri));
...
\copy (SELECT 's'||synonym_id as synonym_id,'tid'||tid as tid,'HasSynonym' as label FROM public.synonym) TO '../neo4j-community-3.4.1/import/gcm_entities/has_synonym.csv' WITH (DELIMITER '+', FORMAT CSV);
-------SYNONYM
...
\copy (SELECT 'x'||reference_id as reference_id,'tid'||tid as tid,'HasXRef' as label FROM public.reference) TO '../neo4j-community-3.4.1/import/gcm_entities/has_reference.csv' WITH (DELIMITER '+', FORMAT CSV);
-------REFERENCE
...
\copy (select 'tid'||tid_parent as tid_parent,'tid'||tid_child as tid_child,rel_type,'HasRelationship' as label from public.relationship) TO '../neo4j-community-3.4.1/import/gcm_entities/has_relationship.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (rel_type));
------PAIRS
