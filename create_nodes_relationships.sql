...
--------DATASET
\copy (SELECT 'da'||dataset_id as dataset_id,regexp_replace(name, E'\\n','') as name,data_type,format,assembly,is_ann, 'Dataset' as label  FROM public.dataset) TO '../neo4j-community-3.4.1/import/gcm_entities/dataset.csv' WITH (DELIMITER '+', FORMAT CSV, FORCE_QUOTE (name, data_type,format,assembly));
--------CASE
